...
class PGA(nn.Module):
    ...
    def perform_iter(self,h,wa,wd,sum_rate,iter_num):
        perform_proj = self.config.proj_iter_rate == 1 or (iter_num % self.config.proj_iter_rate == 0 and iter_num>0) or iter_num == self.num_iter - 1#Perform Projection according to proj rate and always perform on last iter
        ...
        wa = self.wa_projection(wa_t,wd,h,self.num_iter - 1 == iter_num)
        ...
        for i in range(self.config.B):
            if iter_num == 0 or iter_num == 2 :
                ...
                full_grad = True
            if self.config.mu_matrix:
                wd_t[i] = wd[i].clone().detach() + self.hyp[iter_num][i + 1][:self.config.L,:self.config.N] * self.grad_wd(h[i], wa[0], wd[i],full_grad) # gradient ascent
            else:
                wd_t[i] = wd[i].clone().detach() + self.hyp[iter_num][i + 1] * self.grad_wd(h[i], wa[0], wd[i],full_grad) # gradient ascent
    ...
    def grad_wa(self, h, wa, wd):
        ...
        if self.pga_type == 'Classic' or (not(self.config.Wa_G_I) and not(self.config.Wa_G_Ones)):
            ...
            f2 = sum(torch.transpose(h, 2, 3) @ torch.transpose(torch.eye(self.config.N).reshape((1, 1, self.config.N, self.config.N)
                                                                             ), 2, 3)
                                                                             @ h.conj() @ wa.conj() @ wd.conj() @
                                                                             torch.transpose(wd, 2, 3)) / self.config.B
